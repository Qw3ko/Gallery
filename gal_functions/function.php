...
function add($table_name, $file_type, $file, $myfile_name, $file_name)
{
    ...
    $sql = "INSERT INTO $table_name (typ, designation, thum_des, myfile_name) VALUES ('$file_type', '$file_name', '$file', '$myfile_name')";
...
function addSmall($file_type, $file_name, $comment, $user, $date)
{
    /* соединение с базой данных */
    $link = connect();
    ...
    //Добавление записи в базу данных тип файла и временное имя файла на нашем сервере (во время запроса)
    $sql = "INSERT INTO small (typ, designation, address, comment, user, date) VALUES ('$file_type', '$file_name', '$address', '$comment', '$user', '$date')";
    mysqli_query($link, $sql);//запрос к базе данных на добавление записи
    /* закрываем соединение */
    mysqli_close($link);
}
...
function getComment($files)
{
    $link = connect();
    $comment_find = "SELECT comment FROM small WHERE designation = '$files'";
    $query = mysqli_query($link, $comment_find) or die(mysqli_error($link));
    while ($result = mysqli_fetch_array($query)) {
        echo "{$result['comment']}";
      }
}
/* Получение пользователя */
function getUser($files)
{
    $link = connect();
    $user_find = "SELECT user FROM small WHERE designation = '$files'";
    $query = mysqli_query($link, $user_find) or die(mysqli_error($link));
    while ($result = mysqli_fetch_array($query)) {
        echo "{$result['user']}";
      }
}
/* Получение даты */
function getDateNow($files)
{
    $link = connect();
    $date_find = "SELECT date FROM small WHERE designation = '$files'";
    $query = mysqli_query($link, $date_find) or die(mysqli_error($link));
    while ($result = mysqli_fetch_array($query)) {
        echo "{$result['date']}";
      }
}